...
//Have divider return an Array with the first item in the array being the evens array (all the even values from nums) and the second item in the Array being the odds array(all the odd values
// from nums).
...
function divider(numbersArray) {
    ...
    var both = [
        [],
        []
    ]; //to make two separate arrays within an array 2 empty arrays needed to be created within an array.

    for (var i = 0; i < numbersArray.length; i++) {
        if (numbersArray[i] % 2 !== 0) { //modulus is checking for any remainder unequal to zero. zero mean sit is even. anything else means odd.
            odds.push(numbersArray[i]); //if statement is true, push to odd array
        } else {
            evens.push(numbersArray[i]); //otherwise, push to even array
        }
    }
    both[0] = evens; //first empty array is index 0
    both[1] = odds; //second empty array is index 1
    return both;
}
...
// Your job is to write a function named finder that will get a random number (by invoking getRandomArbitrary), then loop through the array (that will be passed in as a parameter) to see if
